...
{
	if (n.mantissa == NULL)
		throw runtime_error("Uninitializated number.");
...
bool operator==(Intln& n1, Intln& n2)
{
	if (n1.mantissa == NULL || n2.mantissa == NULL) 
		throw runtime_error("Uninitializated number");

	if ((strcmp(n1.mantissa, n2.mantissa) == 0) && (n1.exponent == n2.exponent))
		return true;
	return false;
}
...
{
	if (mantissa == NULL || right.mantissa == NULL)
		throw runtime_error("Uninitializated number.");
...
{
	if (left.mantissa == NULL || right.mantissa == NULL)
		throw runtime_error("Uninitializated number.");
